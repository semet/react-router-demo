...
import { type InfiniteScrollTableProps } from './type'

export const InfiniteScrollTable = <T,>(props: InfiniteScrollTableProps<T>) => {
  const {
    columns,
    data = [],
    state,
      ...
      {hasData && state?.pagination && (
        <>
          ...
          <div className="text-sm tracking-wide text-slate-500">
            Showing {table.getRowModel().rows.length.toString()} of {totalData}{' '}
            data
          </div>
        </>
      )}
